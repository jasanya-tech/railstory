...
class Posting_model extends CI_Model
{
   ...
   {
      $insertPosting = $this->db->insert('posting', $data);
      if ($insertPosting == false) return false;
      return true;
   }
   ...
   {
      $this->db->select('posting.id, posting.title, posting.seo_title, posting.content, posting.featured, posting.choice, posting.thread, posting.id_category, posting.photo, posting.is_active, posting.date, category.category_name, category.slug');
      $this->db->from('posting');
      $this->db->join('category', 'category.id = posting. id_category');
