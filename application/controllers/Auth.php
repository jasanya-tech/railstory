	...
	public function register()
	{
		$inputValidation = array(
			array(
				...
				'errors' => array(
					'required' => '%s must be filled in',
					'regex_match' => '%s must be a minimum of 3 characters and alpha characters',
				),
			),
			array(
				'field' => 'username',
			...
			array(
				...
				'errors' => array(
					'required' => '%s must be filled in',
					'valid_email' => '%s entered is incorrect',
				),
			),
			array(
				'field' => 'password',
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$insertUser = $this->auth_model->register($this->input->post());
			if ($insertUser == false) {
				$this->session->set_flashdata('message', "register failed someting wrong");
				return redirect('/auth/register');
			}
			$this->session->set_flashdata('message', "register success, please login by member");
	...
	public function login()
	{
		...
		if ($this->form_validation->run() === TRUE) {
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember)) {
				...
				$roleUserLogin = get_role($user['user_id']);
